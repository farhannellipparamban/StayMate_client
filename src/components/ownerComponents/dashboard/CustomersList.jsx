...
import { Customers, CustomersBlock } from "../../../api/ownerApi";
...
const CustomersList = () => {
  ...
  const blockUnblockCustomer = async (customerId, status) => {
    try {
      const res = await CustomersBlock(customerId, status);
      if (res.status === 200) {
        let updatedData = customers.map((customer) => {
          let customerData = { ...customer };
          if (customerData._id === customerId) {
            customerData.isBlocked = !status;
          }
          return customerData;
        });
  ...
  const customersInSinglePage = filteredData.slice(firstIndex, lastIndex);
        ...
        <div className="mx-auto w-full px-4 py-8 sm:px-8">
          <div className="flex items-center justify-between pb-6">
            <div className="pb-6 font-serif">
              <h2 className="font-bold text-gray-700 ">Customers List</h2>
              <span className="text-xs text-gray-500">
                View accounts of visited Customers
              </span>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="text-black">
                  {customersInSinglePage.length > 0 ? (
                    customersInSinglePage.map((data) => (
                      <tr key={data?._id}>
                        ...
                        <td className="px-4 py-4 bg-gray-300 border-gray-200 border-b">
                          {data?.isVerified ? (
                            <span className="rounded-full bg-green-300 px-3 py-1 text-xs font-semibold text-green-900">
                              Verified
                            </span>
                          ) : (
                            <span className="rounded-full bg-red-300 px-3 py-1 text-xs font-semibold text-red-900">
                              Not Verified
                            </span>
                          )}
                        </td>
