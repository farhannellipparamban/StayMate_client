...
import { ownerForgetPassword } from "../../api/ownerApi";
...
const OwnerForgetPass = () => {
  ...
  const onSubmit = async () => {
    try {
      setLoading(true);
      const res = await ownerForgetPassword(values.email);
      console.log(res);
      if (res.status === 200) {
            ...
            <div className="w-full px-4 md:px-8 max-w-lg space-y-6 bg-gray-200 rounded-3xl py-8 md:py-16">
              ...
              <div className="text-xs md:text-sm text-gray-900 flex flex-col md:flex-row items-center justify-between">
                <p
                  ...
                  className="text-gray-800 cursor-pointer hover:text-red-500 inline-flex items-center mb-2 md:mb-0"
                >
                  <svg
...
export default OwnerForgetPass;
